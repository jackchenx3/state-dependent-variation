...
### Configuration at matched first two trait moments

ORG-STATE-SHAPE-064 returned to the original 002 cohort and all 3,072 paired rows. Each generation-5 O and R population was reshaped in individual-row space, preserving its own centroid, full denominator-N trait covariance and initial mean quadratic loss. If X is the 32-by-2 trait matrix, mu its column mean and U a fixed orthonormal Helmert basis for the subspace perpendicular to the constant vector, the transformed state is

`X_shape = 1 mu + U Q U^T (X - 1 mu)`.

Q is a 31-by-31 orthogonal matrix obtained from a standard-normal matrix by two-pass modified Gram-Schmidt with positive column norms. One prospectively fixed seed per history, family and trial supplied Q, shared across regimes, O/R source states and later rules. This is a random row-space transformation, not a rotation of trait axes or an independent Gaussian population. No matrix was selected using outcomes. The supplement gives the exact sampler and invariant identity.

Only four reshaped continuations per row were generated, through generation 25: 12,288 new paths. The four stored native continuations supplied controls. Every individual then received the same assigned later O or R orientation, with the original proposal parameters and sequential selection operator unchanged. Restored checkpoint random states shared subsequent normal and survival draws across the new branches and reused the original continuation streams. There was no new training or independent training replication. Matching applies at generation 5; later moments may diverge.

For each native or reshaped condition, J is the O-minus-R later-rule performance effect from the O-source state minus that effect from the R-source state. The primary DELTA is J_native minus J_shape in structured histories at generation 25. Its working prediction was negative, indicating attenuation of the previously negative interaction. All eight existing target families, including the uniform-direction reference, received equal weight within each history after averaging eight trials. The ALL8 panel is a fixed equal-weight comparison, not a natural distribution of environmental tasks. One frozen 2,000-by-24 history-resampling array was shared across every cell, family and regime. All 380 intervals are approximate pointwise exploratory 95% intervals; family comparisons do not have simultaneous coverage. G_MEAN, the average native-minus-shaped performance across the four branches, is a separate companion outcome. No primary, family, horizon or seed was changed after outcomes.
...
### Configuration beyond initial mean and covariance made a small contribution

The prespecified structured-history DELTA was -0.740782 percentage points (95% interval -1.015989 to -0.448922), supporting its negative working prediction. The native interaction was -32.720150 points and the reshaped interaction -31.979368 points. Thus reshaping modestly weakened the interaction while leaving most of its magnitude present. The contrast tests the specified intervention on empirical configuration; it is not a percentage of uniquely mediated causation.

| Quantity, percentage points | Structured histories | Isotropic histories |
|---|---:|---:|
| Native interaction J | -32.720150 [-34.580356, -30.990435] | -33.833290 [-35.923423, -31.833441] |
| Reshaped interaction J | -31.979368 [-33.831282, -30.243071] | -33.534208 [-35.708612, -31.516643] |
| Native-minus-reshaped interaction DELTA | -0.740782 [-1.015989, -0.448922] | -0.299083 [-0.666843, 0.050407] |
| Average native-minus-reshaped performance G_MEAN | 0.243637 [0.176620, 0.311369] | 0.287749 [0.200407, 0.372488] |

The isotropic DELTA remained unresolved. The direct structured-minus-isotropic DELTA was also unresolved: -0.441700 points [-0.974416, 0.086306]. Separate significance classifications therefore do not establish a regime difference. Native configurations had a small positive mean-performance advantage over reshaped ones in both regimes, a different claim from the interaction primary. The direct regime difference for this performance companion was unresolved.

Structured-family DELTA intervals were negative at 15, 30, 45 and 90 degrees, but unresolved at 0, 60 and 75 degrees and in the uniform-direction family. Isotropic-history family intervals were unresolved except for their uniform-direction reference. Three of 24 structured history-block primary means had the opposite sign. All conditional results and observed performance declines remain in the compact grid and diagnostics.

![Figure 7. Native-minus-reshaped interaction across all fixed families and pooled contrasts.](../figures/conditional_delta.png)

Figure 7. Native-minus-reshaped interaction across all fixed families and pooled contrasts. Negative DELTA denotes attenuation of the negative native interaction. ALL8 uses equal family weights. The bottom row is the direct regime contrast. Intervals are pointwise exploratory 95% intervals, not simultaneous statements about every family.

All 6,144 transformed checkpoints met the fixed absolute 1e-12 initial matching tolerance; subsequent mean and covariance divergence is shown in Figure S1. A separate supervisor reconstruction checked all 380 estimates and the prospectively selected history-0 block: 512 new paths and 10,240 updates. All survivor indices matched. Floating arithmetic differences stayed within the frozen tolerance, including a maximum 3.00e-15 sampler discrepancy and 4.44e-16 selected-coordinate discrepancy. This internal audit adds no independent training cohort.
